...
% ============================================================ who you read from
\begin{frame}{Does it matter \emph{who} you read from?}
  \vspace{0.3em}
  \footnotesize
  So far every reader hit \textbf{rank 0}. That is the worst case: one owner,
  eleven readers. What if the traffic is spread instead?

  \vspace{0.35em}
  \centering
  \begin{tikzpicture}[scale=0.58, every node/.style={font=\scriptsize}]
    % all vs 1
    \node[font=\scriptsize\bfseries] at (2.6,3.5) {all vs 1 \ (\texttt{hotspot})};
    \draw[fill=myred!30, draw=myred!80!black, rounded corners] (2.0,2.2) rectangle (3.2,2.9);
    \node at (2.6,2.55) {rank 0};
    \foreach \i/\x in {1/0.0, 2/1.3, 3/2.6, 4/3.9, 5/5.2} {
      \draw[fill=gray!12, draw=gray, rounded corners] (\x,0.2) rectangle (\x+1.1,0.9);
      \node[font=\tiny] at (\x+0.55,0.55) {rank \i};
      \draw[->, myred, thick] (\x+0.55,0.9) -- (2.6,2.2);
    }
    \node[gray, font=\tiny] at (2.6,-0.35) {one door, everybody queues};
    % all vs all
    \node[font=\scriptsize\bfseries] at (11.4,3.5) {all vs all \ (\texttt{ring})};
    \foreach \i/\x in {0/8.2, 1/9.5, 2/10.8, 3/12.1, 4/13.4} {
      \draw[fill=mygreen!25, draw=mygreen!80!black, rounded corners] (\x,2.2) rectangle (\x+1.1,2.9);
      \node[font=\tiny] at (\x+0.55,2.55) {rank \i};
      \draw[fill=gray!12, draw=gray, rounded corners] (\x,0.2) rectangle (\x+1.1,0.9);
      \node[font=\tiny] at (\x+0.55,0.55) {rank \i};
    }
    \foreach \x/\y in {8.2/9.5, 9.5/10.8, 10.8/12.1, 12.1/13.4} {
      \draw[->, mygreen!70!black, thick] (\x+0.55,0.9) -- (\y+0.55,2.2);
    }
    \node[gray, font=\tiny] at (11.4,-0.35) {one door each, nobody queues};
  \end{tikzpicture}

  \vspace{0.3em}
  \raggedright
  \begin{itemize}\itemsep1pt
    \item identical kernel, identical thread count --- \textbf{only the target changes}
    \item \texttt{random}: each \emph{warp} draws its own target. Per warp, not per thread:
          a warp issues one load for all 32 lanes, so per-thread targets would
          also measure intra-warp fan-out and confuse the two effects.
  \end{itemize}
