  ...
  \begin{itemize}\itemsep1pt
    \item Two access patterns:
      \begin{itemize}\footnotesize\itemsep0pt
        \item \texttt{random}: each lane starts from a different random position in the same cycle, resulting in approximately 32 load chains per warp.
...
\begin{frame}{Throughput Benchmark -- Broadcast}
  \vspace{0.2em}
  \footnotesize
  In every cluster, every thread of ranks $1\ldots N{-}1$ reads \textbf{rank 0}'s shared memory.\\
  ...
  \begin{columns}[T]
    \begin{column}{0.5\linewidth}
      \centering
      \textbf{One cluster}\\[0.2em]
      \includegraphics[width=\linewidth]{images/throughput-broadcast.png}\\[0.2em]
      \scriptsize 1 cluster of $N$ blocks: only $N$ SMs busy.
    \end{column}
    \begin{column}{0.5\linewidth}
      \centering
      \textbf{Whole GPU}\\[0.2em]
      \includegraphics[width=\linewidth]{images/throughput-broadcast-gpu.png}\\[0.2em]
      \scriptsize $48/N$ clusters of $N$ blocks side by side: all 48 SMs busy.
    \end{column}
  \end{columns}
\end{frame}

% ============================================================ the result
\begin{frame}{Throughput Benchmark -- Ring}
  \vspace{0.2em}
  \footnotesize
  In every cluster, every thread of rank $k$ reads \textbf{rank $k{+}1$} (mod $N$): one reader per owner.\\
  1 block per SM, 256 threads per block.
  ...
  \begin{columns}[T]
    \begin{column}{0.5\linewidth}
      \centering
      \textbf{One cluster}\\[0.2em]
      \includegraphics[width=\linewidth]{images/throughput-ring.png}\\[0.2em]
      \scriptsize 1 cluster of $N$ blocks: only $N$ SMs busy.
    \end{column}
    \begin{column}{0.5\linewidth}
      \centering
      \textbf{Whole GPU}\\[0.2em]
      \includegraphics[width=\linewidth]{images/throughput-ring-gpu.png}\\[0.2em]
      \scriptsize $48/N$ clusters of $N$ blocks side by side: all 48 SMs busy.
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Throughput Benchmark -- Summary}
  \footnotesize
  ...
  \begin{tabular}{lrrl}
    \toprule
    scope & \textbf{coalesced} & random & what sets the limit \\
    \midrule
    one owner SM & \textbf{9.8 GB/s} & 0.67 GB/s & the owner: 128\,B every $\sim$31\,cy $\approx$ 4.1\,B/cy \\
    one GPC (12 SMs) & \textbf{$\sim$53 GB/s} & 4.3 GB/s & the GPC's SM-to-SM network \\
    whole chip (48 SMs) & \textbf{$\sim$206 GB/s} & 16 GB/s & 4 GPCs, independent \\
    \bottomrule
  \end{tabular}
  ...
  \raggedright
  \begin{itemize}\itemsep2pt
    \item \textbf{coalesced} is limited by \emph{bytes}, \textbf{random} by \emph{requests}
          (one every $\sim$14\,cy per owner, only 4\,B each): $15\times$ apart.
    \item The owner here is SM\,0, the worst position in the GPC; the best-placed
          owner reaches $\sim$17\,GB/s ($\sim$7\,B/cy).
    \item The GPC network saturates with $\sim$6 SMs reading: beyond that, adding SMs
          only splits the same $\sim$53\,GB/s.
  \end{itemize}
\end{frame}
